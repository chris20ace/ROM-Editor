...
Zoom out to see everything, or select an area or map to focus it. Paint terrain and edit people directly without changing pages. Save world writes the edited maps together. For an exact connection, use **Connected view**. It shows the selected map and its immediate neighbors at the offsets used by the game, with complete rectangular tile grids. Selecting Route 103 or Route 110 opens this view automatically; an overview seam button opens the aligned join for editing. Paint across compatible maps, edit people, and Undo on the same canvas. Use **World overview** to return to all places without losing pending edits. Emerald's local route connections do not all fit one global grid, so the overview still flags incompatible loop closures. Neither view changes source dimensions or travel links.
