...
Zoom out to see everything, or select an area or map to focus it. Paint terrain and edit people directly without changing pages. Save world writes the edited maps together. Emerald's original route loop does not close on a single rectangular grid. The world view gently slopes a strip of Route 103’s water to align Route 110 while preserving the Oldale entrance. All source tiles remain present and editable, and map sizes, event coordinates, and travel links are unchanged. **World alignment · details** explains this display projection and previews the original connection. If custom map sizes or links invalidate the safe adjustment, the canvas shows an amber warning instead of forcing it.
