...
Start in the [World canvas](http://127.0.0.1:8765/worldmap). Every source map appears once, organized into **Towns & cities**, **Routes**, **Caves & landmarks**, and **Other areas**. This includes homes, shops, gyms, cave floors, underwater maps, secret bases and special facilities. Complete maps are arranged in labeled sections. Walking connections inside each section use their original offsets; links between sections identify where those same game connections continue. Separate rooms and floors remain in area groups on the same canvas.

Zoom out to see everything, or select an area or map to focus it. Paint terrain and edit people directly without changing pages. Save world writes the edited maps together. Every map retains its full rectangular tile grid. A brush stroke can continue across touching maps with compatible artwork; separated sections are reached by panning or selecting the destination map. Pending edits and Undo history stay available throughout the canvas.

The sections organize the editor around the game's map dimensions and connections. Their spacing does not create, remove or change travel links. Emerald's local route connections do not all fit one global grid, so section boundaries make those relationships explicit while preserving every source tile. Use **Connections** to change where a route actually leads in the game.
