...
**Character appearance** groups matching overworld and battle artwork under each character. Choose a character card, then switch between **Overworld**, **Battle front**, **Battle back**, and its other sheets above the pixel editor. Trainer matches come from map battle scripts and the game's trainer graphics tables. A shared overworld sheet can appear in several trainer groups; editing it affects every character using that sheet. NPCs without a battle portrait remain available with that absence labeled.

Choose **Add space** on the world canvas to add blank rows or columns above, below, left or right of a map. Set each side’s size and choose the background or selected brush tile for the new ground. Preview the enlarged terrain, then save. Existing terrain stays at its world position; events are shifted within the expanded map, and walking offsets are updated. New tiles use passable collision and elevation 3; terrain behavior can still restrict movement.

To combine maps, choose **Move maps (G)**, select two or more adjoining boxes, then choose **Combine boxes**. The boxes must fill one rectangle without gaps or overlaps and use the same primary and secondary terrain tilesets. Choose which map’s name, settings, border and callbacks to keep. Matching wild encounters are preserved; for differing tables, explicitly choose the kept map’s encounters for the whole result. The preview checks connections and references before saving terrain, events, incoming doors and supported walking links as one map. Internal walking seams disappear. Unsupported connections or shared layouts/events must be resolved first.

Expansion and combination save backed-up source edits and preserve other canvas positions. **Saved history** restores the source and positions together when positions have not changed since; later manual position edits are kept. Story scripts with literal coordinates, warp numbers or object IDs need review after reshaping; map callbacks from different maps are not automatically combined. Map-size limits still apply. Combining maps can change numeric map IDs, so existing game saves may be incompatible; build and test with a new save.
...
Use **Move maps (G)** to move one or several map boxes around the canvas. **Shift-click** (or Ctrl-click) boxes to add or remove them from the selection, or choose **Select multiple** and drag a rectangle around them. Drag any highlighted box to move the entire group, keeping their spacing. **Esc** deselects; **Space + drag** pans. These positions save automatically in `.workbench/world-positions.json`; **Undo map move** reverses the entire group’s position change together. Use **Connections → Preview connections → Apply to game** on the world canvas to make walking connections follow that arrangement. Source maps are rectangular, so route splitting follows a full vertical or horizontal tile boundary. Each half needs at least 8 columns for a vertical split or 7 rows for a horizontal split to support the game’s connection buffers; freehand pixel cuts remain available inside a map through Cut line.

Open **Connections** on the world canvas to show actual game links: green matching walking edges, orange displaced edges, and blue lines between exact doorway tiles. Select any map and use **Show destination** to jump to an entrance’s destination, or **Edit doorway** to change it in the existing event editor. Dynamic/scripted destinations are labeled separately; original out-of-bounds entrance records are shown for review and are not automatically rewritten.

After moving boxes, choose **Preview connections** and **Apply to game**. The default updates towns and routes; **Selected map boxes** supports other explicitly selected maps, including caves. Touching edges create reciprocal walking connections with offsets from your arrangement; old links between scoped boxes that no longer touch are removed. The preview lists the changes and blocks overlaps or conflicting exits. Doors, stairs, dive/emerge and links outside the scope keep their destinations. Terrain and collision remain unchanged, so crossing tiles must be walkable. Every canvas position is preserved, and **Saved history** can undo the saved connections. Moving a box alone still saves its placement; applying connections is the step that updates playable travel.
